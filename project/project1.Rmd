...
head(BDW)
```
To re-tidy my data, I used pivot_longer to retake those columns and put them into 1 column with its values representing the percent of driversinvolved in fatal accidents who were speeding. 
I then removed the NAs from that column to condense the dataset so that there weren't rows of the same state with empty values
```{R}
...
```
Here, the original data set didn't have a state column, but rather had the row names as states, so here I converted the state row names into a column entitled 'state'
```{R}
USArrests <- USArrests
...
```{R}
murderstat <- cut((full_mutate$Murder), breaks = c(0, 10, 20), labels = c("low", "high"))
```
