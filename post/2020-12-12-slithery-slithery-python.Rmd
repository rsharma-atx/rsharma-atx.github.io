...
---
<center>
![Python: Snake or Code?]![](/post/2020-12-12-slithery-slithery-python_files/close-up-of-a-snake-with-a-laptop-X2KFDN.png){width=300px height=200px} 
...
We can make a code chunk in R:

```{R}
library(reticulate)
husband <- "McDreamy"
```
And then we can also make a code chunk in python:
```{python}
wife = "Meredith Grey"

#we can acess our R-defined objects from r in python like so:

print(r.husband)
```

And you can also access python defined objects from R: 
```{R}
cat(c(husband,py$wife))
```
And there you have it! These are just some of the concepts we learned and mastered throughout our semester in R and python. 